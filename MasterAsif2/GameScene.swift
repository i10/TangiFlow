...
//               print("")
        ...
        for passiveTangible in self.passiveTangibles{
            if passiveTangible.state == .initializedAndRecognized{
            var node = PassiveTangibleEx.getTangible(by: passiveTangible.identifier)?.node
                if node!.rotationMode{
                if let arc = node?.getTempArc(){
                    
                    if let edge = arc.tempEdge{
                        print("i am calling toooo")
                        self.graph?.aim(edge: edge, angle: passiveTangible.zRotation,rotationPoint: arc)
                    }
                    }
